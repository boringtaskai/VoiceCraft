...
RUN conda update -y -n base -c conda-forge conda && \
    ...
    conda run -n voicecraft mfa model download --github_token $github_token dictionary english_us_arpa && \
    conda run -n voicecraft mfa model download --github_token $github_token acoustic english_us_arpa && \
